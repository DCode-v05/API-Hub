...
import type { DiagnosticDTO, ProposalDTO, RunEvent, StageSourceKind, SurfaceDTO, TestResult } from '../events';
...
/**
 * Re-render surfaces from a version's pinned IR with the CURRENT generator. Surfaces are pure
 * functions of the IR, so distribution (host/CLI/publish) projects fresh rather than trusting the
 * stored snapshot — which may have been generated by an older projection (e.g. before the hostable
 * http-server.mjs existed). Deterministic: same IR ⇒ same surfaces.
 */
export function projectSurfaces(ir: Ir): SurfaceDTO[] {
  return project(ir).surfaces.map((s) => ({
    kind: s.kind,
    dir: s.dir,
...
export async function executePipeline(source: SourceRef, opts: ExecuteOptions = {}): Promise<PipelineResult> {
  ...
  result.tests = { tests, passed: tests.length - failed, failed };
  emit?.({ t: 'test', ms: Date.now() - tt, tests, passed: tests.length - failed, failed });

  result.ok = failed === 0;
  result.ms = Date.now() - t0;
  emit?.({ t: 'done', ok: result.ok, ms: result.ms });
  return result;
}
